...
Our philosophy for AI integration is grounded in user control, architectural flexibility, confidentiality, and responsible adoption. AI capabilities should remain disabled by default and be enabled only when users expressly choose to use them. Customers should retain full freedom to select the LLM, model, or provider that best fits their technical, security, regulatory, and operational requirements, rather than being locked into a vendor-imposed choice. To reduce the risk of data leakage, customers should also be able to use self-hosted, private, or on-premises models where appropriate, ensuring that sensitive metadata, schemas, data models, and other customer artifacts are not unnecessarily exposed to external AI services, retained by third parties, or used to train models without the customer’s explicit authorization. Finally, while we believe in keeping a human in the loop, we do not view human oversight as a superficial approval step where users merely glance at algorithmic output under time pressure, with incomplete information and institutional pressure to move fast. Meaningful human oversight should involve informed judgment, clear accountability, and the ability to challenge, validate, or reject AI-generated outputs, rather than simply adding a human signature to automation bias.
...
![Ontology-based AI semantic layer](<lib/Ontology-based AI semantic layer.png>)
...
In the book [Agentic Design Patterns](<https://irp.cdn-website.com/ca79032a/files/uploaded/Agentic-Design-Patterns.pdf> "target=\"\_blank\""), Marco Argenti, CIO, Goldman Sachs shares the following thoughts which we found insightful and inspiring:
