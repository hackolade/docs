...
| \--outputType=\< **jsonschema** \| jsondata yamldata \| script \| ddl \> | Y | Specify the type of output (JSON Schema, sample JSON data, sample YAML data, or script) \[default: jsonschema\] This also allows output of the script corresponding to the target of the specified model, e.g.: CQL for Cassandra, HQL for Hive, etc... |
